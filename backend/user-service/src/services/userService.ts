...
const userSignIn = async (name:string, email:string, password:string) =>{
    try {
...
const userLogIn = async (email:string, password:string) =>{
    try {
        ...
        if(!passMatch) return {success:false, message:'Incorrect credentials'}
...
const changePass = async (email:string, password:string) =>{
    try {
        const user=await userRepository.findByEmail(email)
        if(!user) return {success:false, message:'Email doesnt exist'}
        const hashedPass=await bcrypt.hash(password, 10)
...
const getUserName = async (userid:string) =>{
    try {
        ...
        if(!user) return {success:false, message:'Server Error please try again'}
        return {succesS:true, user:user}
    } catch (error) {
        return {success:false, message:'Server error please try again'}
    }
...
const editUser = async (userid:string, name:string, image:string) =>{
    try {
